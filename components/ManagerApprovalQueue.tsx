...
export default function ManagerApprovalQueue({ moduleId, onAction }: { moduleId: string, onAction?: () => void }) {
  ...
  const handleAction = async (id: number, action: 'approved' | 'rejected') => {
    ...
    if (!error) {
      if (tableName === 'mr_procurement' && action === 'approved') {
        try {
          ...
          {items.map(item => (
            <div key={item.id} className="bg-gray-800/50 border border-gray-700 p-5 rounded-xl transition-all duration-300 hover:border-gray-600">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2 gap-4">
                <div>
                  <span className={`text-xs font-bold px-2 py-1 rounded-md uppercase tracking-wider ${item.agent_status === 'pending_manager_review' ? 'bg-yellow-500/20 text-yellow-400' :
              ...
              {expandedItems[item.id] && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 animate-fade-in-up">
                  ...
                  {item.agent_metadata && (
                    <div className={`p-5 rounded-2xl text-sm flex flex-col h-full shadow-2xl backdrop-blur-md relative overflow-hidden ${(item.agent_metadata.fraud_risk === 'High' || item.agent_metadata.ai_risk_level === 'High' || item.agent_metadata.ai_risk_level === 'Critical' || item.agent_metadata.ai_absenteeism_risk === 'High Risk' || item.agent_metadata.ai_absenteeism_risk === 'High')
                      ...
                      <div className={`absolute -top-24 -right-24 w-48 h-48 rounded-full blur-3xl opacity-20 pointer-events-none ${(item.agent_metadata.fraud_risk === 'High' || item.agent_metadata.ai_risk_level === 'High' || item.agent_metadata.ai_risk_level === 'Critical' || item.agent_metadata.ai_absenteeism_risk === 'High Risk' || item.agent_metadata.ai_absenteeism_risk === 'High') ? 'bg-red-500' : 'bg-emerald-500'
                        }`}></div>
                      ...
                      <div className="space-y-4 flex-grow z-10">
                        ...
                        {(item.agent_metadata.fraud_risk || item.agent_metadata.ai_risk_level || item.agent_metadata.ai_absenteeism_risk) && (
                          <div className="flex justify-between items-center bg-black/20 p-3 rounded-xl border border-white/5">
                            ...
                            <span className={`font-bold px-3 py-1 rounded-md text-xs tracking-widest uppercase shadow-sm ${(item.agent_metadata.fraud_risk === 'High' || item.agent_metadata.ai_risk_level === 'High' || item.agent_metadata.ai_risk_level === 'Critical' || item.agent_metadata.ai_absenteeism_risk === 'High Risk' || item.agent_metadata.ai_absenteeism_risk === 'High')
                        ...
                        <div className="pt-2">
                          ...
                          <div className={`p-4 rounded-xl leading-relaxed text-gray-200 border-l-4 shadow-inner ${(item.agent_metadata.fraud_risk === 'High' || item.agent_metadata.ai_risk_level === 'High' || item.agent_metadata.ai_risk_level === 'Critical' || item.agent_metadata.ai_absenteeism_risk === 'High Risk' || item.agent_metadata.ai_absenteeism_risk === 'High')
