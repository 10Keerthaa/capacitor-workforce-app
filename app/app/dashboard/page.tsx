...
export default function DashboardPage() {
  ...
  const handleManagerDecision = async (id: any, decision: string, table: string) => {
    ...
    // HUMAN IN THE LOOP: Send email/PDF ONLY if the manager manually clicked "Approve"
    if (table === 'mr_procurement' && decision === 'Approved') {
      try {
        await fetch("https://one0xworkforce-ai.onrender.com/api/procurement/approve", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id }),
        });
      } catch (err) {
        console.error("Failed to trigger approval workflow:", err);
      }
    }
  ...
  const runAgentOnPending = async () => {
    ...
    if (activeTab === 'supervisor') {
      ...
      try {
        const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/supervisor", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            highRiskFinances,
      ...
      for (const claim of pendingClaims) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/petty-cash", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: claim.id,
      ...
      for (const item of pendingProcurements) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/procurement", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
      ...
      for (const item of pendingWork) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/work-output", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
      ...
      for (const item of pendingTools) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/tools", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
      ...
      for (const item of pendingManpower) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/manpower", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
      ...
      for (const item of pendingCampBoss) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/camp-boss", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
      ...
      for (const item of pendingOnboarding) {
        try {
          const res = await fetch("https://one0xworkforce-ai.onrender.com/api/agents/onboarding", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              id: item.id,
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-slate-200 font-sans selection:bg-teal-500/30 overflow-x-auto">
      ...
      <div className="w-full overflow-x-hidden">
        ...
        <div className="relative overflow-hidden border-b border-white/5 bg-black/40 backdrop-blur-3xl pt-12 pb-8">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full md:w-[800px] h-[300px] bg-indigo-500/20 blur-[120px] rounded-full pointer-events-none" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            ...
            <button
              ...
              disabled={isAuditing}
              className="shrink-0 group relative inline-flex justify-center items-center gap-2 overflow-hidden rounded-full bg-gradient-to-r from-indigo-500 to-violet-600 px-8 py-4 text-sm font-bold text-white transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:pointer-events-none shadow-[0_0_40px_-10px_rgba(99,102,241,0.5)]"
            >
              {isAuditing ? (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 mt-8">
          <div className="flex gap-4 border-b border-white/10 pb-4 overflow-x-auto custom-scrollbar">
            <button 
              onClick={() => { setActiveTab('supervisor'); setIsMobileDetailView(false); }}
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            {activeTab === 'supervisor' && supervisorReports.length > 0 && (
              <div className="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <div className={`w-full lg:w-1/3 flex flex-col gap-3 max-h-[800px] overflow-y-auto custom-scrollbar pr-2 ${isMobileDetailView ? 'hidden lg:flex' : 'flex'}`}>
                  ...
                  {supervisorReports.map((report) => {
                    ...
                    return (
                      <div 
                        key={report.id}
                        onClick={() => { setSelectedReportId(report.id); setIsMobileDetailView(true); }}
                ...
                <div className={`w-full lg:w-2/3 sticky top-4 ${!isMobileDetailView ? 'hidden lg:block' : 'block'}`}>
                  {(() => {
                    ...
                    return (
                      <div className="relative p-1 rounded-3xl bg-gradient-to-br from-indigo-500/30 via-purple-500/10 to-transparent shadow-2xl transition-all duration-300">
                        ...
                        <button 
                          ...
                          className="lg:hidden mb-4 flex items-center justify-center gap-2 text-indigo-400 hover:text-indigo-300 font-bold px-6 py-3 w-full bg-indigo-500/10 rounded-2xl border border-indigo-500/20"
                        >
                          <ChevronDown className="w-5 h-5 rotate-90" /> Back to Reports
                        </button>
                ...
                <div className="flex-1 w-full space-y-4">
                  <div className="flex flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        <h3 className="font-semibold text-white text-lg truncate">{item.materialName || item.title || 'Unknown Material'} <span className="text-teal-400">(Qty: {item.quantity || 'N/A'})</span></h3>
                        <p className="text-slate-400 text-sm flex items-center gap-2 truncate">
                          <span className="font-medium text-slate-300">MR #{item.mrNo}</span>
                          <span className="w-1 h-1 rounded-full bg-slate-600"></span>
                          <span>Required: {item.requiredDate || 'N/A'}</span>
                          <span className="w-1 h-1 rounded-full bg-slate-600"></span>
                    ...
                    <div className="space-y-4 relative z-10">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="w-full">
                          ...
                          <p className="text-slate-300 text-sm leading-relaxed mb-3">{item.ai_reason}</p>
                          
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3">
                            {item.ai_recommended_vendor && (
                              <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-lg p-3">
                                <p className="text-indigo-300 text-[10px] font-bold uppercase tracking-wider mb-1">💡 Recommended Vendor</p>
                                <p className="text-white text-sm font-medium">{item.ai_recommended_vendor}</p>
                              </div>
                            )}
                            <div className="bg-teal-500/10 border border-teal-500/20 rounded-lg p-3">
                              <p className="text-teal-300 text-[10px] font-bold uppercase tracking-wider mb-1">💰 Est. Unit Price</p>
                              <p className="text-white text-sm font-medium">${item.unitPrice || '0.00'}</p>
                            </div>
                          </div>
                        </div>
                      </div>
